...
```bash
bunx instant-cli push schema --yes
```
...
```bash
bunx instant-cli push schema --rename 'posts.author:posts.creator stores.owner:stores.manager' --yes
```
...
```bash
bunx instant-cli push perms --yes
```
...
// On client
import { init } from '@instantdb/svelte'; // or your relevant Instant SDK
...
```tsx
import { id } from '@instantdb/svelte'; // or your relevant Instant SDK
...
```tsx
import { AppSchema } from '$lib/instant.schema';
...
Run `bunx instant-cli query '{ posts: {} }' --admin` to query your app. A context flag is required: `--admin`, `--as-email <email>`, or `--as-guest`. Also supports `--app <id>`.
